...
def trainModel_scale(model, features, data, nFoldList, nTest, scaler, output):
    for i in nFoldList:
        ...
        preprocessor = ColumnTransformer(transformers=[("sca", scaler, features)])

        my_pipeline = Pipeline(steps=[("preprocessor", preprocessor), ("model", model)])

        cv = ShuffleSplit(test_size=i, n_splits=nTest)
        scores = cross_val_score(my_pipeline, X, y, cv=cv, scoring="precision",)
...
neighborhood_location_lat_lng = df_criminal.loc[
    :, ["Colonia", "location_lat", "location_lng"]
]
neighborhood_location_lat_lng.drop_duplicates(
    subset=["Colonia"], keep=False, inplace=True
)
...
for _, station in estaciones_df.iterrows():
    ...
    stations.append(
        {"id": station_id, "hasCrime": station_id in stations_with_crime_near}
    )
...
trips_sum = (
    df_mibici_gender_filtered["Origen_Id"].value_counts()
    + df_mibici_gender_filtered["Destino_Id"].value_counts()
)

trips_sum = pd.DataFrame(trips_sum)

stations_crimes_and_trips = stations.merge(
    trips_sum, right_index=True, left_on="id", how="right"
)
