...
##MiBici
mibici_dataset_direct_url = create_onedrive_directdownload(
    "https://1drv.ms/u/s!AllbB8dY7-XlonYbKR5K_UxVhb5N?e=2dyg0K"
)
estaciones_direct_url = create_onedrive_directdownload(
    "https://1drv.ms/u/s!AllbB8dY7-XlondVkj-i2oSp7X0M?e=fq1fOv"
)
...
df = pd.read_csv(
    mibici_dataset_direct_url, parse_dates=date_cols, date_parser=pd.to_datetime,
)
print("Done.")
...
df.rename(
    columns={"latitud_x": "latitud_origen", "longitud_x": "longitud_origen"},
    inplace=True,
)
df.rename(
    columns={"latitud_y": "latitud_destino", "longitud_y": "longitud_destino"},
    inplace=True,
)
...
df_primero.rename(
    columns={"latitud_origen": "latitud", "longitud_origen": "longitud"}, inplace=True
)
df_segundo.rename(
    columns={"latitud_destino": "latitud", "longitud_destino": "longitud"}, inplace=True
)
...
df_new.rename(
    columns={"latitud": "location_lat", "longitud": "location_lng"}, inplace=True
)
...
criminal_incidence_direct_url = create_onedrive_directdownload(
    "https://1drv.ms/u/s!AllbB8dY7-Xlonl9IYCBVwrS6wug?e=8JW4cU"
)
neighborhoods_lat_lng_direct_url = create_onedrive_directdownload(
    "https://1drv.ms/u/s!AllbB8dY7-XlonigcNaWfXwaC5dC?e=ggEhzD"
)
...
df_criminal = df_criminal.merge(
    lat_lng, left_on="Colonia", right_on="colonia", how="left"
)
df_criminal.drop(
    ["Mes", "Clave_Mun", "colonia", "query", "status"], axis=1, inplace=True
)
...
df_criminal.loc[
    df_criminal.Delito == "ROBO A CUENTAHABIENTES", "y_roboCuentahabientes"
] = 1
...
indexNames = df_criminal[
    (df_criminal.location_lat < 20.3257581)
    | (df_criminal.location_lat > 20.9982375)
    | (df_criminal.location_lng < -103.6650327)
    | (df_criminal.location_lng > -103.0809884)
].index
...
def trainAndPredict(model, features, data, scaler, output, categorical_cols, X_test):
    ...
    scaler = Pipeline(steps=[("sca", scaler)])
...
data = df_criminal[
    (df_criminal["y_roboCuentahabientes"] == 1)
    | (df_criminal["y_homicidioDoloso"] == 1)
][all_rows].dropna()
scaler = MinMaxScaler()
...
y_pred = trainAndPredict(
    model, features, data, scaler, output, categorical_cols, X_test
)
